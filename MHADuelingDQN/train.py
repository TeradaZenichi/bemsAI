...
from MHADuelingDQN.model import MHADuelingDQN  # import do novo pacote MHADuelingDQN
...
def train(cfg_path: str):
    ...
    models_dir = os.path.join("Models", "MHADuelingDQN")
    ...
    pbar = trange(1, cfg['num_episodes'] + 1, desc='Training MHADuelingDQN')
    for episode in pbar:
        # resample noise once per episode
        model.reset_noise()
        ...
        if episode % cfg.get('save_interval', 10) == 0:
            fname = f"{name_no_ext}_ep{episode}{ext}"
            torch.save(model.state_dict(), os.path.join(models_dir, fname))

        pbar.set_postfix({'soc_ini': f'{soc_ini:.2f}', 'ε': f'{epsilon:.3f}', 'R': f'{total_reward:.2f}', 'C': f'{total_cost:.2f}'})

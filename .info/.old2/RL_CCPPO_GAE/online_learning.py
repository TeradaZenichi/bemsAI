...
def run_episode_collect(agent, env, device, soc_init=0.5):
    """
    Run a single episode, returning total energy cost, total reward,
    and a detailed step-by-step DataFrame.
    """
    ...
    while not done:
        ...
        total_reward += reward
        # Store relevant variables for each step
...
def sequential_test(agent, test_days, hp, device, steps_per_day, soc_init=0.5):
    """
    Test 1: Runs sequentially over the training dataset days,
    passing the final SoC of one episode as the initial SoC for the next.
    Returns a concatenated DataFrame for all days, and final SoC.
    """
    soc = soc_init
    ...
    for idx, d in enumerate(test_days):
        ...
        dfs.append(df)
        soc = env.soc  # update for next day
...
def standard_test(agent, test_days, hp, device, steps_per_day, soc_values=[0.0, 0.5, 1.0]):
    """
    Test 2: Runs over the specified test_days from the test dataset,
    with each SoC in soc_values. Returns a concatenated DataFrame for all days/SoCs.
    """
    costs_per_soc = []
    rewards_per_soc = []
    dfs = []
...
def main():
    ...
    # Ensure the output directory exists before any save
    ...
    # Dynamically set number of test days from config (default: 30)
    ...
    # --- NEW: Track SoC final for sequential tests ---
    last_final_soc = 0.5  # Default SoC if first model

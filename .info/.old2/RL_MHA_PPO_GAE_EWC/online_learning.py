...
from RL_CCPPO_GAE_EWC.train import HyperParameters, PPOTrainer
...
def main():
    ...
    online_path = 'RL_CCPPO_GAE_EWC/online_learning.json'
    save_dir = "models/online/ppo_ewc"
    fisher_dir = "models/ewc"
    ...
    prev_fisher = None
    prev_theta = None
    last_final_soc = 0.5  # Initial SoC
    ...
    for i, (train_days, val_days) in enumerate(zip(train_days_list, val_days_list)):
        ...
        # Load Fisher/theta_star if not first window
        if i > 0:
            fisher_path = os.path.join(fisher_dir, f"fisher_theta_day{train_days[0]-1}.pt")
            if os.path.exists(fisher_path):
                d = torch.load(fisher_path)
                prev_fisher = d['fisher']
                prev_theta = d['theta_star']
                print(f"Loaded EWC penalty from day {train_days[0]-1}")
            else:
                prev_fisher, prev_theta = None, None
        ...
        # NEW: load last_final_soc from previous test metrics if exists (for test1)
        if i > 0:
        ...
        fisher_path = os.path.join(fisher_dir, f"fisher_theta_day{train_days[0]}.pt")
        torch.save({'fisher': fisher, 'theta_star': theta_star}, fisher_path)

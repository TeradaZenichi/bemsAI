...
class HyperParameters:
    def __init__(self, param_path: str, model_path: str):
        ...
        self.n_heads         = agent_cfg.get('n_heads', 2)
        self.n_steps         = agent_cfg.get('n_steps', 8)
        ...
        self.experiment_type = model_cfg.get('experiment_type', 'Default')
...
class PPOTrainer:
    def __init__(self, hp, train_days=None, val_days=None, data_dir=None, obs_keys=None,
        ...
        self.critic_opt = torch.optim.Adam(self.agent.critic.parameters(), lr=hp.critic_lr)
    ...
    def collect_rollout_buffer(self, buffer_size):
        ...
        states   = torch.zeros((buffer_size, n_steps, obs_dim), dtype=torch.float32, device=self.device)
        ...
        # Initialize buffer with the first state repeated
        window = deque([torch.as_tensor(state, dtype=torch.float32, device=self.device)] * n_steps, maxlen=n_steps)
        done = False

        while collected < buffer_size:
            state_seq = torch.stack(list(window)).unsqueeze(0)  # [1, n_steps, obs_dim]
            act, lp, _ = self.agent.sample_action(state_seq)
            act_np = act.detach().cpu().numpy() if isinstance(act, torch.Tensor) else act
            nxt, r, done, info = self.env.step(act_np)
            # Update window
            window.append(torch.as_tensor(nxt, dtype=torch.float32, device=self.device))

            states[collected]   = torch.stack(list(window))
    ...
    def penalty_lwf(self, states):
        ...
        return torch.nn.functional.mse_loss(mu, target_mu)
    ...
    def ppo_update(self, states, actions, old_lps, ret, adv, entropy_coef):
        for epoch in range(self.hp.ppo_epochs):
            ...
            for i in range(0, len(states), self.hp.minibatch_size):
                ...
                vf_loss = torch.nn.functional.mse_loss(self.agent.critic(mb_st).view(-1), mb_ret)
    ...
    def run_episode(self, env, soc_init=None):
        ...
        states   = torch.zeros((max_steps, n_steps, obs_dim), dtype=torch.float32, device=self.device)
        ...
        # Inicializa a janela com estado atual
        window = deque([torch.as_tensor(state, dtype=torch.float32, device=self.device)] * n_steps, maxlen=n_steps)
        done = False
        t = 0
        ...
        with torch.inference_mode():
            while not done and t < max_steps:
                state_seq = torch.stack(list(window)).unsqueeze(0)
                act, lp, _ = self.agent.sample_action(state_seq)
                act_np = act.detach().cpu().numpy() if isinstance(act, torch.Tensor) else act
                nxt, r, done, info = env.step(act_np)
                window.append(torch.as_tensor(nxt, dtype=torch.float32, device=self.device))

                states[t]   = torch.stack(list(window))
        ...
        with tqdm(range(self.num_rollouts), desc="Rollouts (train)", leave=False) as pbar:
            for rollout in pbar:
                ...
                with torch.no_grad():
                    ...
                    nxt_v = self.agent.evaluate_state_value(states[-1].unsqueeze(0)).item()
...
def run_episode_for_plot(env, agent, device, n_steps):
    obs_dim = env.observation_space.shape[0]
    max_steps = getattr(env, 'episode_length', 1000)
    state = env.reset()
    window = deque([torch.as_tensor(state, dtype=torch.float32, device=device)] * n_steps, maxlen=n_steps)
    done = False
    t = 0
    p_bess, p_grid, p_pv, p_load, socs, times = [], [], [], [], [], []
    with torch.inference_mode():
        while not done and t < max_steps:
            state_seq = torch.stack(list(window)).unsqueeze(0)
            action, _, _ = agent.sample_action(state_seq)
            act_np = action.detach().cpu().numpy() if isinstance(action, torch.Tensor) else action
            nxt, _, done, info = env.step(act_np)
            window.append(torch.as_tensor(nxt, dtype=torch.float32, device=device))
            state = nxt
            socs.append(env.soc)
            times.append(info.get('time', t))
            p_bess.append(info.get('p_bess', 0.0))
...
if __name__ == "__main__":
    ...
    model_path = 'MHA_PPO_REG/model.json'      # Novo path!
    ...
    save_dir = f"Results/MHA/{hp.experiment_type}"
    ...
    val_plot = run_episode_for_plot(trainer.eval_env, trainer.agent, trainer.device, hp.n_steps)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'ppo_val_episode_output.png'))
    plt.show()

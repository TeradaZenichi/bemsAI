...
class SACTrainer:
    def __init__(self, hp, train_days, val_days):
        ...
        self.target_entropy = -float(hp.act_dim)  # Default SAC value, can be overwritten above
    ...
    def run_episode(self, episode):
        ...
        episode_reward = 0
        q1_vals, q2_vals, actor_loss_vals, critic_loss_vals, alpha_vals_local, entropies = ([] for _ in range(6))
        val_reward = ""
        for t in range(self.hp.steps_per_episode):
            ...
            if len(self.buffer) > self.hp.batch_size:
                ...
                if update_info is not None:
                    ...
                    entropies.append(update_info.get("entropy", np.nan))
            obs = obs_next
        ...
        stats = {
            ...
            "entropy": np.nanmean(entropies) if entropies else np.nan,
    ...
    def update_stats_buffers(self, stats_buffers, stats):
        ...
        stats_buffers["entropy"].append(stats["entropy"])
        stats_buffers["target_entropy"].append(stats["target_entropy"])
    ...
    def update_target_entropy_if_needed(self, episode_reward):
        if self.rewards_window.is_full():
            ...
            if episode_reward < moving_avg:
                self.target_entropy += self.hp.target_entropy_delta
            else:
                self.target_entropy -= self.hp.target_entropy_delta
            self.target_entropy = np.clip(
                self.target_entropy,
                self.hp.target_entropy_clip_min,
                self.hp.target_entropy_clip_max
            )
    ...
    def _update_progress_bar(self, pbar, stats, val_reward, best_val, patience_counter):
        pbar.set_postfix({
            ...
            "TargetH": f"{stats['target_entropy']:.2f}",

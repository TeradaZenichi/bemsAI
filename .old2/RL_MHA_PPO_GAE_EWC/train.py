...
from RL_MHA_PPO_GAE_EWC.model import PPOAgent  # Modelo com multihead attention
...
class HyperParameters:
    def __init__(self, param_path: str, model_path: str):
        ...
        self.lambda_ewc      = agent_cfg.get('lambda_ewc', 100)
        self.n_heads         = agent_cfg.get('n_heads', 4)          # << ATENÇÃO
        self.n_steps         = agent_cfg.get('n_steps', 6)          # << ATENÇÃO: tamanho da janela
...
class PPOTrainer:
    def __init__(self, hp, 
                 ...
                 num_rollouts=1, prev_fisher=None, prev_theta=None):
        self.hp = hp
        ...
        self.state_dim = len(self.obs_keys)
        ...
        # Instantiate agent with MHA and window
        self.agent = PPOAgent(
    ...
    def _init_state_window(self, first_state):
        # Fill with the first state (repeat)
        window = torch.stack([first_state for _ in range(self.n_steps)], dim=0)
        return window

    def _update_state_window(self, window, new_state):
        new_window = torch.cat([window[1:], new_state.unsqueeze(0)], dim=0)
        return new_window

    def collect_rollout_buffer(self, buffer_size):
        states   = torch.zeros((buffer_size, self.n_steps, self.state_dim), dtype=torch.float32, device=self.device)
        ...
        first_state = torch.as_tensor(state, dtype=torch.float32, device=self.device)
        state_window = self._init_state_window(first_state)

        done = False

        while collected < buffer_size:
            st_window = state_window
            act, lp, _ = self.agent.sample_action(st_window.unsqueeze(0))  # [1, n_steps, state_dim]
            act_np = act.detach().cpu().numpy() if isinstance(act, torch.Tensor) else act
            nxt, r, done, info = self.env.step(act_np)

            states[collected]   = st_window.detach()
            ...
            total_r += r
            state = nxt
            new_state_tensor = torch.as_tensor(state, dtype=torch.float32, device=self.device)
            state_window = self._update_state_window(state_window, new_state_tensor)
    ...
    def ppo_update(self, states, actions, old_lps, ret, adv, entropy_coef):
        ...
        for epoch in range(self.hp.ppo_epochs):
            ...
            for i in range(0, len(states), self.hp.minibatch_size):
                ...
                # ------- EWC loss --------
                if self.prev_fisher is not None and self.prev_theta is not None:
                    ewc_loss = 0.0
                    for n, p in self.agent.named_parameters():
                        if n in self.prev_fisher and n in self.prev_theta:
                            ewc_loss += (self.prev_fisher[n] * (p - self.prev_theta[n]).pow(2)).sum()
                    loss = loss + self.hp.lambda_ewc * ewc_loss
                # -------------------------
    ...
    def run_episode(self, env, soc_init=None):
        ...
        states   = torch.zeros((max_steps, self.n_steps, obs_dim), dtype=torch.float32, device=self.device)
        ...
        with torch.inference_mode():
            while not done and t < max_steps:
                st_window = state_window
                act, lp, _ = self.agent.sample_action(st_window.unsqueeze(0))
                act_np = act.detach().cpu().numpy() if isinstance(act, torch.Tensor) else act
                nxt, r, done, info = env.step(act_np)

                states[t]   = st_window.detach()
    ...
    def train_and_validate(self):
        ...
        rollout_states, rollout_actions = None, None
        ...
        with tqdm(range(self.num_rollouts), desc="Rollouts (train)", leave=False) as pbar:
            for rollout in pbar:
                ...
                rollout_states, rollout_actions = states, actions
                ...
                with torch.no_grad():
                    ...
                    nxt_v = self.agent.evaluate_state_value(
                        states[-1].unsqueeze(0)  # last state window
                    ).item()
                ...
                # Early stopping: only activate for difficulty >= 1.0
                if diff >= 1.0:
        ...
        fisher = self.compute_fisher_on_rollout(rollout_states, rollout_actions)
        theta_star = {n: p.detach().clone() for n, p in self.agent.named_parameters() if p.requires_grad}

        return t_r, v_r, fisher, theta_star

    def compute_fisher_on_rollout(self, rollout_states, rollout_actions, sample_n=500):
        idx = torch.randint(0, len(rollout_states), (min(sample_n, len(rollout_states)),))
        states = rollout_states[idx].to(self.device)
        actions = rollout_actions[idx].to(self.device)
        ...
        loss = log_probs.sum()
        loss.backward()
        fisher = {n: p.grad.detach() ** 2 for n, p in self.agent.named_parameters() if p.grad is not None}
        return fisher


def run_episode_for_plot(env, agent, device, n_steps=None, obs_dim=None):
    """
    Runs an evaluation episode using sliding window input for the MHA agent.
    """
    # If not provided, infer window and obs size from env or agent
    if n_steps is None:
        n_steps = agent.n_steps if hasattr(agent, 'n_steps') else 4
    if obs_dim is None:
    ...
    # Initialize state window by repeating the first state
    first_state = torch.as_tensor(state, dtype=torch.float32, device=device)
    state_window = torch.stack([first_state for _ in range(n_steps)], dim=0)  # shape: [n_steps, obs_dim]

    with torch.inference_mode():
        while not done and t < max_steps:
            st_win = state_window.unsqueeze(0)  # shape: [1, n_steps, obs_dim]
...
if __name__ == "__main__":
    ...
    fisher_save_path = "models/mha_ewc/fisher_theta_last.pt"

    if os.path.exists(fisher_save_path):
        d = torch.load(fisher_save_path)
        prev_fisher = d['fisher']
        prev_theta = d['theta_star']
    else:
        prev_fisher = None
        prev_theta = None
    ...
    t_r, v_r, fisher, theta_star = trainer.train_and_validate()
    ...
    # Save Fisher/Theta_star for next window
    os.makedirs("models/ewc", exist_ok=True)
    torch.save({'fisher': fisher, 'theta_star': theta_star}, fisher_save_path)

    # Run validation and plot results (NOW passes n_steps and obs_dim)
    val_plot = run_episode_for_plot(
        trainer.eval_env,
        trainer.agent,
        trainer.device,
        n_steps=hp.n_steps,

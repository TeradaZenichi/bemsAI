...
# --- Ajuste para o novo diretório ---
target_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.append(target_path)

from env import EnergyEnvContinuous
from RL_CCPPO_GAE_SI.model import PPOAgent   # Novo path!
...
class HyperParameters:
    def __init__(self, param_path: str, model_path: str):
        ...
        # Parâmetros para Synaptic Intelligence
        self.lambda_si       = agent_cfg.get('lambda_si', 1.0)
        self.epsilon_si      = agent_cfg.get('epsilon_si', 0.1)
...
class PPOTrainer:
    def __init__(self, hp, train_days=None, val_days=None, data_dir=None, obs_keys=None, device=None, timestep=None, num_rollouts=1,
                 prev_si_omega=None, prev_si_theta=None):
        self.hp = hp
        ...
        # --------- SI: Inicialize variáveis para a penalidade ---------
        self.si_omega = {n: torch.zeros_like(p, device=self.device) for n, p in self.agent.named_parameters() if p.requires_grad}
        self.si_prev_params = {n: p.data.clone() for n, p in self.agent.named_parameters() if p.requires_grad}
        self.si_w = {n: torch.zeros_like(p, device=self.device) for n, p in self.agent.named_parameters() if p.requires_grad}
        # Se recebeu omega/theta de fora, atualize os valores
        if prev_si_omega is not None:
            for n in self.si_omega.keys():
                self.si_omega[n] = prev_si_omega[n].clone()
        if prev_si_theta is not None:
            for n in self.si_prev_params.keys():
                self.si_prev_params[n] = prev_si_theta[n].clone()
        # -------------------------------------------------------------
    ...
    def ppo_update(self, states, actions, old_lps, ret, adv, entropy_coef):
        # -------- SI: Salve os parâmetros antes do update --------
        ...
        for epoch in range(self.hp.ppo_epochs):
            ...
            for i in range(0, len(states), self.hp.minibatch_size):
                ...
                # ------ SI: Penalidade de importância acumulada ------
                for n, p in self.agent.named_parameters():
                    if p.requires_grad:
                        loss = loss + self.hp.lambda_si * (self.si_omega[n] * (p - self.si_prev_params[n]).pow(2)).sum()
                # ----------------------------------------------------
                ...
                # ------ SI: Acumule delta para cada parâmetro --------
                for n, p in self.agent.named_parameters():
                    if p.requires_grad:
                        delta = (p.data - self.si_prev_params[n])
                        if p.grad is not None:
                            self.si_w[n] += -p.grad.data * delta
                        self.si_prev_params[n] = p.data.clone()
                # ----------------------------------------------------

    def update_si_omega(self):
        # Após o treinamento de cada janela, chame isso para atualizar as importâncias
        for n, p in self.agent.named_parameters():
            if p.requires_grad:
                denom = (p.data - self.si_prev_params[n]).pow(2) + self.hp.epsilon_si
                self.si_omega[n] += self.si_w[n] / denom
                self.si_w[n].zero_()
                self.si_prev_params[n] = p.data.clone()
    ...
    def train_and_validate(self):
        ...
        # Ao final do treino da janela/tarefa, atualize o omega
        self.update_si_omega()

        # Retorne também os estados SI para o ciclo online
        # self.si_prev_params = snapshot dos pesos finais
        return t_r, v_r, {k: v.clone() for k, v in self.si_omega.items()}, {k: v.clone() for k, v in self.si_prev_params.items()}
...
if __name__ == "__main__":
    ...
    model_path = 'RL_CCPPO_GAE_SI/model.json'      # Novo path!

    hp = HyperParameters(param_path, model_path)
    train_days = [1, 2, 3]
    val_days = [4, 5]

    # Exemplo: para ciclo único (treino offline), começa sem penalidade
    trainer = PPOTrainer(
        hp,
    ...
    # Salve os estados SI para uso futuro (continual learning)
    si_path = os.path.join(save_dir, f"si_state_{'_'.join(map(str, train_days))}.pt")
    torch.save({
        'si_omega': si_omega,
        'si_theta_star': si_theta_star
    }, si_path)
    print(f"SI state saved at: {si_path}")
    ...
    plt.tight_layout()
    plt.savefig('ppo_si_val_episode_output.png')
    plt.show()
